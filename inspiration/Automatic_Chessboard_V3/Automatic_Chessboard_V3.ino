...
void motor(byte direction, int speed, float distance) {
  ...
  if (distance == calibrate_speed)
    step_number = 4;
  else if (direction == LR_BT || direction == RL_TB || direction == LR_TB ||
           direction == RL_BT)
    step_number =
        distance * SQUARE_SIZE * 1.44; //  Add an extra length for the diagonal
  else
...
void black_player_movement() {
  ...
  if (reed_sensor_status_memory[convert_table[arrival_coord_Y]]
                               [arrival_coord_X - 1] == 0)
    white_capturing = 0;
  ...
  //  Diagonal displacement
    ...
    if (departure_coord_X > arrival_coord_X &&
        departure_coord_Y > arrival_coord_Y)
  ...
  //  Kingside castling
  else if (departure_coord_X == 5 && departure_coord_Y == 8 &&
           arrival_coord_X == 7 && arrival_coord_Y == 8) { //  Kingside castling
    motor(R_L, SPEED_SLOW, 0.5);
    ...
    motor(L_R, SPEED_SLOW, 0.5);
             ...
             arrival_coord_X == 3 &&
             arrival_coord_Y == 8) { //  Queenside castling
    motor(R_L, SPEED_SLOW, 0.5);
    motor(B_T, SPEED_SLOW, 2);
    electromagnet(false);
  ...
  reed_sensor_status_memory[convert_table[departure_coord_Y]]
                           [departure_coord_X - 1] = 1;
  reed_sensor_status_memory[convert_table[arrival_coord_Y]]
                           [arrival_coord_X - 1] = 0;
  reed_sensor_status[convert_table[departure_coord_Y]][departure_coord_X - 1] =
      1;
